class Form::VariablecostForm < Form::Base
  ...
  super attributes
      ...
      self.variablecost_values = variablecosts.map { |nariablecost| VariablecostValue.new(variablecost_id: variablecost.id) } unless variablecost_values.present?
  end
  
  def variablecost_values_attributes=(attributes)
    self.variablecost_values = attributes.map do |_, variablecost_value_attributes|
      From::VariablecostValue.new(variablecost_value_attributes).tap { |v| puts v }
    end
  end
  ...
  def save
    ...
    VariablecostValue.trtansaction {
